...
async function transferOwner(leagueId: number, displayName: string, userId: string) {
  const checkOwner = await db.select({ isOwner: leagueMembers.isOwner })
    .from(leagueMembers)
    .where(and(
      eq(leagueMembers.league, leagueId),
...
async function bootMember(leagueId: number, displayName: string, userId: string) {
  const checkOwner = await db.select({ isOwner: leagueMembers.isOwner })
    .from(leagueMembers)
    .where(and(
      eq(leagueMembers.league, leagueId),
...
export async function promoteMember(leagueId: number, displayName: string) {
  ...
  const userStatus = await db.select({ isAdmin: leagueMembers.isAdmin, isOwner: leagueMembers.isOwner })
    .from(leagueMembers)
    .where(and(
      eq(leagueMembers.league, leagueId),
      eq(leagueMembers.displayName, displayName)));
  ...
  if (userStatus[0].isAdmin) await transferOwner(leagueId, displayName, user.userId);
