...
export async function GET(
...
): Promise<NextResponse<{ events: TribeEvent[]; updates: TribeUpdates }>> {
    const seasonName = params.name;
    const searchParams = req.nextUrl.searchParams;
    const tribeName = searchParams.get("name");

    const tribeEventData = db.select({
        tribe: tribes.name,
    ...
    const tribeUpdateData = db.select({
        tribe: tribes.name,
    ...
    const [tribeEvents, tribeUpdatesRaw] = await Promise.all([tribeEventData, tribeUpdateData]);

    const tribeUpdates = tribeUpdatesRaw.reduce((acc, { tribe, castaway, episode },) => {
