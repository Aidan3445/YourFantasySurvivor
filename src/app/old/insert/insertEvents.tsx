  ...
  for (let { id, name } of data) {
    ...
    try {
      ...
      await Promise.all(fetchEpisodes.map(async (episode) => {
        ...
        const ep = await db.select({ id: episodes.episodeId }).from(episodes).where(and(eq(episodes.seasonId, id), eq(episodes.number, episode.number)));
        ...
        await Promise.all(episode.advsFound.map(async ({ name }) => {
          ...
          const newEventId = await db.insert(baseEvents).values(newEvent).returning({ id: baseEvents.baseEventId });

          const castawayId = (await db.select({ id: castaways.castawayId }).from(castaways).where(eq(castaways.name, name)))[0]?.id ?? 0;

          const newEventCastaway = {
            event: newEventId[0]?.id,
            reference: castawayId
          } as CastawayInsert;
        ...
        let newEventId = await db.insert(baseEvents).values(newEvent).returning({ id: baseEvents.baseEventId });
        ...
        await Promise.all(tribeTribe1st.map(async (tribe1st: { name: { name: string }, onModel: 'Tribes' | 'Survivors' }) => {
          ...
          const tribeId = (await db.select({ id: tribes.tribeId }).from(tribes).where(eq(tribes.name, tribe1st.name.name)))[0]?.id ?? 0;
          const newEventTribe = {
            event: newEventId[0]?.id,
            reference: tribeId
          } as TribeInsert;
        ...
        if (tribeSurvivor1st.length > 0) {
          ...
          await Promise.all(tribeSurvivor1st.map(async (tribe1st: { name: { name: string }, onModel: 'Tribes' | 'Survivors' }) => {
            const castawayId = (await db.select({ id: castaways.castawayId }).from(castaways).where(eq(castaways.name, tribe1st.name.name)))[0]?.id ?? 0;
            const newEventCastaway = {
              event: newEventId[0]?.id,
              reference: castawayId
            } as CastawayInsert;
        ...
        await Promise.all(tribeTribe2nd.map(async (tribe2nd: { name: { name: string }, onModel: 'Tribes' | 'Survivors' }) => {
          ...
          const tribeId = (await db.select({ id: tribes.tribeId }).from(tribes).where(eq(tribes.name, tribe2nd.name.name)))[0]?.id ?? 0;
          const newEventTribe = {
            event: newEventId[0]?.id,
            reference: tribeId
          } as TribeInsert;
        ...
        if (tribeSurvivor2nd.length > 0) {
          ...
          await Promise.all(tribeSurvivor2nd.map(async (tribe2nd: { name: { name: string }, onModel: 'Tribes' | 'Survivors' }) => {
            const castawayId = (await db.select({ id: castaways.castawayId }).from(castaways).where(eq(castaways.name, tribe2nd.name.name)))[0]?.id ?? 0;
            const newEventCastaway = {
              event: newEventId[0]?.id,
              reference: castawayId
            } as CastawayInsert;
        ...
        newEventId = await db.insert(baseEvents).values(newEvent).returning({ id: baseEvents.baseEventId });
        ...
        await Promise.all(episode.tribeUpdates.map(async (update: { tribe: { name: string }, survivors: { name: string }[] }) => {
          const tribeId = (await db.select({ id: tribes.tribeId }).from(tribes).where(eq(tribes.name, update.tribe.name)))[0]?.id ?? 0;
          const castawayids = await Promise.all(update.survivors.map(async (survivor) => {
            return (await db.select({ id: castaways.castawayId }).from(castaways).where(eq(castaways.name, survivor.name)))[0]?.id ?? 0;
          }));
