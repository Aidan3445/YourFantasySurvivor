    ...
    <Form {...form}>
      <form className={className} action={catchUpdate} onChange={() => setUnsaved(true)}>
        <Tab value='base' valid={valid} unsaved={unsaved} ownerLoggedIn={ownerLoggedIn}>
          ...
          <article className='hidden col-span-2 lg:block'>
            <h3 className='text-lg font-semibold'>Base Events</h3>
            Base events are added automatically when they
            occur in an episode. <br />
            You can customize the points each event is worth. <br /><p className='text-sm italic'>
        ...
        <Tab value='weekly' valid={valid} unsaved={unsaved} ownerLoggedIn={ownerLoggedIn}>
          ...
          <article className='hidden col-span-2 lg:block'>
            ...
            <ul className='pl-4 h-40 text-sm list-disc light-scroll'>
              <li>
                <h3 className='inline font-semibold'>Votes</h3>:
                After each episode, members vote on specific events.
                Points are awarded to those who receive the most votes.</li>
              <li>
                <h3 className='inline font-semibold'>Predictions</h3>:
        ...
        <Tab value='season' valid={valid} unsaved={unsaved} ownerLoggedIn={ownerLoggedIn}>
          ...
          <article className='hidden col-span-2 lg:block'>
            ...
            or before the final episode of the season. <br /> <p className='text-sm italic'>
              Predictions can be tied to other events and poins will be earned for each
...
function Tab({ children, value, valid, unsaved, ownerLoggedIn }: TabProps) {
  return (
    <TabsContent value={value}>
      <section className='gap-2 md:grid md:grid-cols-5 md:max-w-screen-sm'>
        {children}
        {ownerLoggedIn ?
          <div className='flex flex-col col-start-5 row-start-2 gap-2 mt-auto mb-4 text-center'>
            {unsaved && <p className='text-sm font-semibold text-red-900'>Unsaved Changes</p>}
            <Button disabled={!valid} type='submit'>Save</Button>
          </div> :
          <h3 className='col-span-2 col-start-4 row-start-2 mt-auto mb-4 text-sm italic'>Only the owner can edit league rules</h3>}
      </section>
    </TabsContent >
  );
}
